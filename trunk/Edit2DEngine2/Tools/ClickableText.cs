...
namespace Edit2DEngine
{
    public class ClickableText
    {
        ...
        public int Width
        {
            get { return (int)_spriteFontMouseIn.MeasureString(Text).X; }
        }

        public int Height
        {
            get { return (int)_spriteFontMouseIn.MeasureString(Text).Y; }
        }
        #endregion
        ...
        public void Draw(SpriteBatch spriteBatch, Color color)
        {
            ...
            {
                if (isIn)
                    spriteBatch.DrawString(_spriteFontMouseIn, Text, this.Position, color);
                else
                    spriteBatch.DrawString(_spriteFontMouseOut, Text, this.Position, color);
            }
            else
            {
                if (isIn)
