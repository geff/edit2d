...
namespace Xna.Tools
{
    public partial class CurveControl : UserControl
    {
        ...
        [Description("Turn on or off tool strip in Curve Control.")]
        [DefaultValue(true)]
        ...
        [DefaultValue(typeof(Color), "80, 0, 200, 128")]
        public Color SelectingBoxColor
        {
        ...
        public Color SelectingBoxBorderColor
        {
        ...
        #region Events
        public delegate void TimeLineChangeEventHandler(object sender, int value);
        public event TimeLineChangeEventHandler TimeLineChange;

        public delegate void CurveChangeEventHandler(object sender, EventArgs e);
        public event CurveChangeEventHandler CurveChange;
        #endregion
            ...
            if (editable)
            {
                switch (em)
                {
                    ...
                    case EditMode.Move:
                        BeginUpdateCurves();
                        break;
                    case EditMode.MoveTimeLine:
                        TimeLineChange(this, -1);
                        break;
                }
            }
        }
            ...
            if ((e.Button & MouseButtons.Left) == MouseButtons.Left)
            {
                switch (em)
                {
                    case EditMode.MoveTimeLine:
                        ...
                        Vector2 pos_mtl = curveView.ToUnitCoordinate((float)e.X, (float)e.Y);
                        //this.TimeLine = (int)curveView.ToPixelCoordinate(pos).X;

                        MathHelper.Clamp(pos_mtl.X,
                            -1f * MaximumCurveKeyEntry,
                            MaximumCurveKeyEntry);

                        MathHelper.Clamp(pos_mtl.Y,
                            -1f * MaximumCurveKeyEntry,
                            MaximumCurveKeyEntry);

                        this.TimeLine = (int)pos_mtl.X;

                        TimeLineChange(this, this.TimeLine);
                    ...
                    case EditMode.Move:
                        if (editable)
                        {
                            ...
                            CurveChange(this, new EventArgs());
        ...
        private void addZeroKey_Click(object sender, EventArgs e)
        {
            UpdateCurves(delegate(EditCurve curve)
            {
                curve.AddKey(Vector2.Zero);
            });

            addKeyButton.PerformClick();
        }
        ...
        {
            Color[] colors = new Color[] { Color.Yellow, Color.Brown };
            float len = 50.0f;
        ...
        {
            Vector2 pos = curveView.ToPixelCoordinate((float)TimeLine, 0);
            System.Drawing.Rectangle rec = new System.Drawing.Rectangle((int)pos.X, 0, 5, curveView.Height - 1);
            g.FillRectangle(Brushes.Yellow, rec);
            g.DrawRectangle(Pens.Orange, rec);
        }
        ...
        {
            Brush selected_brush = new SolidBrush(Color.Yellow);
            Brush unselected_brush = new SolidBrush(Color.Black);
        ...
        private void DrawCurves(Graphics g)
        {
            ...
            foreach (EditCurve curve in curves)
            {
                ...
                Color infinityCurveColor = Color.FromArgb((int)c.X, (int)c.Y, (int)c.Z);
        ...
        private Color selectingBoxColor = Color.FromArgb(80, 0, 200, 128);
        private Color selectingBoxBorderColor = Color.Black;
        #endregion
        ...
        private EditCurveView curveTangentsViewMode = EditCurveView.Never;
        ...
        public void ChangeScaling(float scaleX, float scaleY)
        {
            curveView.Scale(new SizeF(scaleX, scaleY));
        }

        public void Frame(float minX, float minY, float maxX, float maxY)
        {
            curveView.Frame(new Vector2(minX, minY), new Vector2(maxX, maxY));
        }
        ...
        private void smoothButton_Click(object sender, EventArgs e)
        {
            if (curves.Count > 0)
            {
                UpdateCurves(delegate(EditCurve curve)
                {
                    curve.SetTangents(EditCurveSelections.TangentOut, EditCurveTangent.Smooth);
                });
            }
        ...
        private void linearButton_Click(object sender, EventArgs e)
        {
            if (curves.Count > 0)
            {
                UpdateCurves(delegate(EditCurve curve)
                {
                    curve.SetTangents(EditCurveSelections.TangentOut, EditCurveTangent.Linear);
                });
            }

            linearButton.Checked = true;
            smoothButton.Checked = false;
